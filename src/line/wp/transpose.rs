use super::{Line, ChordIterator, LineIterator, Directive, TextChordTrans, Transposabel, NotTransposable, Chord, Text, Translation};
                ...
                match item {
                    Chord(c) => {
                        ...
                        for item in ChordIterator::new(&c) {
                            match item {
                                Transposabel(s) => line_new.push_str(SCALE[scale][(chord_to_level(&s)+key_new-key_old).rem_euclid(12) as usize]),
...
impl<I> Iterator for Transpose<I> where I: Iterator<Item = Line> {
    type Item = Line;
...
pub trait IntoTranspose: Iterator {
    fn transpose(self, key: &str) -> Transpose<Self> where Self: Sized + Iterator<Item = Line> {
