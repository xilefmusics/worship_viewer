...
- **BLC-MAST-012:** All accepted uploads finish validation, expansion/transcoding, asset ingestion, and Media mutation before returning. Uploads whose kind does not match existing content are rejected and their staging data is removed.
...
- **BLC-MAST-009:** Configurable staging/final directories, per-kind limits, processing timeouts, and FFmpeg tool paths are validated at startup when media processing is enabled. Missing required tools produce an actionable warning but do not prevent backend startup. Audio/video uploads that require unavailable tooling are rejected before their request bodies are staged with **400** and code `media_processing_unavailable`; client-visible errors do not expose configured command paths. PDF deck uploads are parsed and split synchronously in-process and do not require external PDF tools.
