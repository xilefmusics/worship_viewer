...
import { isValidUrlMediaInput, type UrlMediaKind, urlContent } from '@/lib/media-display'
...
export function CreateMediaDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; onCreated: (id: string) => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return createMedia(queryClient, { title: title.trim(), owner, content: urlContent(kind, url.trim()) })
    },
    onSuccess: (created) => {
    ...
    <Dialog.Root open={open} onOpenChange={close}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 grid max-h-[90dvh] w-[min(32rem,calc(100%-2rem))] -translate-x-1/2 -translate-y-1/2 gap-5 overflow-y-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-[var(--shadow-elevated)]">
          ...
          <MediaFields title={title} kind={kind} url={url} owner={owner} teams={teams} userId={user?.id} showTeam={teams.length > 1} disabled={mutation.isPending} onTitleChange={setTitle} onKindChange={setKind} onUrlChange={setUrl} onOwnerChange={setOwner} />
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="button" disabled={mutation.isPending || teamsPending || teams.length === 0} onClick={() => { setError(''); mutation.mutate() }}>{mutation.isPending ? t('common.load') : t('media.actions.create')}</Button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
